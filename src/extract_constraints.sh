...
echo
echo "Syntax: extract_constraints.sh database_folder query_protein_model mapconstraints_folder lovoalign_executable function_to_define_constraints dtol"
echo
echo "Functions available FLAT_HARMONIC LINEAR_PENALTY USOG"
echo
echo "User must provide dtol. Recommended settings dtol=3.0 [default] for library creation and dtol=1.0 for refinement stage."
}
...
python3.7 $extract_constraints -database $database/ -query query_aligned.pdb -func $5 -dtol $6
